...
**RAM: the image stack, not the JS heap.** What moves this process's footprint
is libvips, the native core of `sharp`. Nothing in the repo imports it any
more — the share-card renderer that did is gone — but Astro's `/_image`
endpoint still reaches for it at runtime, because the SSR home page optimises
its photo per request. That memory is native, so `bun --smol` never sees it.
Measured on the built server: boot ~86MB, ~103MB once the home and event pages
have rendered, ~160MB after the photo's 15 variants have each been encoded
once — and it stays there. libvips is threaded and glibc holds a 64MB arena per
allocating thread, so `docker-entrypoint.sh` sets `MALLOC_ARENA_MAX=2`; without
it the same run lands at ~178MB. Anything that pulls a rasteriser back into this
process is a decision to argue for, not a convenience.
