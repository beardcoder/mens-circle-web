...
**RAM: no image library at runtime, and keep it that way.** What used to move
this process's footprint was libvips, the native core of `sharp` — memory
allocated outside the Bun heap, which is why `bun --smol` never touched it and
why it never came back down. Two things reached for it and neither does now:
the generated share card (removed) and Astro's `/_image` endpoint, which
resized the home page's photo per request because that page renders on demand.
Those variants are encoded during the build instead
(`astro-integrations/hero-images.mjs`), so **no page this server renders asks
for an image**. Measured on the built server: boot ~89MB, ~100MB with the home
page, an event page and all nine image variants served — and it stays there,
against ~160MB before.

Astro still registers `/_image`, so a hand-written request to it pulls libvips
in (~+27MB, and ~189MB over fifteen such requests). `docker-entrypoint.sh` caps
that with `MALLOC_ARENA_MAX=2` — libvips is threaded and glibc keeps a 64MB
arena per allocating thread — which brings the same run to ~159MB. That is
insurance; nothing on the site takes that path. **Adding `<Image>`/`<Picture>`
to an on-demand page puts libvips back in the process for the life of the
container, silently.** Prerendered pages are free to use `astro:assets` — Astro
emits their variants at build time. On-demand pages go through the integration;
`tests/frontend-performance.test.ts` holds that line.
...
**Share pictures — one standing poster, nothing generated.** Every page's
`og:image` is `public/images/og-default.png`, a real 1200x630 file;
`SeoHead.astro` reaches for it whenever a page passes no image of its own, and
`buildEventMeta` names the same file so the JSON-LD cannot disagree with the
meta tags. The event pages used to render their own card per request
(`/event/<slug>/card.png`, satori → SVG → sharp): it was removed because the
only long-lived process on this box had to keep a native image stack resident
for it, and the date a forwarded link has to answer for is already in
`og:title` and in the description, which WhatsApp and Facebook show as text
beside the picture. Don't reintroduce a renderer here without that trade in
hand — see **RAM** above.

The `og:image:width/height` a page declares must match the file, so they are
emitted only when known: `SeoHead.astro` fills in 1200x630 for the default
poster, and a page passing an image of its own passes its size with it. An
admin-entered `image_url` is an arbitrary picture of unknown size, so it rides
along as a second `image` in the JSON-LD and never as the `og:image`.

**Responsive images on on-demand pages — `astro-integrations/hero-images.mjs`.**
`astro:assets` picks where an image is resized by asking whether the page is
prerendered: build time if it is, the `/_image` endpoint per request if it is
not. The home page is not, so its photograph was the last thing keeping an
image library in the web process. The integration encodes the variants during
`astro:config:setup` — so `astro dev`, `astro check` and `astro build` all get
them — writes them to `public/generated/` (gitignored, hence served in dev and
copied into `dist/client` for the build with no manifest patching), and exposes
the finished URLs as the virtual module `virtual:hero-images`, typed in
`src/env.d.ts`. `Hero.astro` renders a plain `<picture>` from it.

Two details: the filename carries eight hex digits of the **source bytes**, so
replacing the photo changes every URL and the `astro:build:done` hook can mark
them `immutable` (the adapter's default for `public/` is a day, revalidated);
and files that no longer belong to the current widths are deleted, because
nothing else ever prunes `public/`.

The encoder settings are measured, not taste — the table in the integration has
sizes and PSNR against Astro's defaults. Short version: AVIF q45 at 4:2:0
(sharp defaults to 4:4:4, which a photograph does not need), WebP q74 at effort
6, JPEG q76 through mozjpeg. About a quarter off every variant for ~1dB. AVIF
`effort` is left at the default on purpose: 6 costs 2.7x the time for 2% of the
bytes, and this is in every cold build.
