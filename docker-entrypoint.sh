...
#
# Recurring tasks (reminders, backups) are NOT started from here — they run
# via scripts/schedule.ts, invoked once a minute by the HOST's own cron
# (Coolify's "Scheduled Task"), not by anything inside this process. See
...
HOST="$ASTRO_HOST" PORT="$ASTRO_PORT" \
  exec bun --smol /app/dist/server/entry.mjs
