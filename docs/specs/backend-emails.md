...
2. If found: promote it to `registered` (this itself does not re-trigger promotion since the guard
   only fires on transition *to* cancelled).
...
routerAdd("POST", "/api/event/register", (e) => {
  ...
  const participant = upsertParticipantByEmail(email, { first_name, last_name, phone: phone_number })
  const existing = findRegistration(participant.id, event.id, { withTrashed: true })
  if (existing && !existing.deleted) {
    return json(409, existing.status === "waitlist"
  ...
  isWaitlist ? sendWaitlistConfirmation(reg, event, participant)
...
onRecordAfterUpdateSuccess((e) => {
  ...
  if (next) {
    next.set("status", "registered"); $app.save(next)
...
onRecordAfterCreateSuccess((e) => { sendNewsletterWelcome(e.record) }, "newsletter_subscribers")

// campaign batch (superuser route):
function sendCampaign(newsletter) {
  newsletter.set("status","sending"); save()
  let count = 0
  ...
  newsletter.set({ status:"sent", sent_at: now(), recipient_count: count }); save()
}
```
...
$app.cron().add("event-reminders", "*/15 * * * *", () => {
  const regs = findActiveRegistrationsForEventsBetween(startOfToday(), endOfTomorrow())
