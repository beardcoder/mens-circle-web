<script lang="ts">
  ...
  function buildIcsString(ev: EventData): string {
    ...
    const stamp = formatICSDate(new Date().toISOString().slice(0, 10), new Date().toISOString().slice(11, 16));
  ...
  function buildGoogleCalendarUrl(ev: EventData): string {
    const formatDate = (date: string, time: string): string => `${date.replace(/-/g, '')}T${time.replace(':', '')}00`;
...
<div class="event-info__calendar">
  ...
  <dialog bind:this={dialogEl} class="calendar-modal" aria-labelledby="calendar-modal-title" onclick={onBackdropClick}>
    <div class="calendar-modal__content">
      <button type="button" class="calendar-modal__close" aria-label="Schließen" onclick={close}>
      ...
      <div class="calendar-modal__buttons">
        <a href={googleUrl} class="btn btn--secondary" target="_blank" rel="noopener" onclick={trackGoogle}>
          Google Calendar
        </a>
        <a href={icsBlobUrl} class="btn btn--secondary" download="maennerkreis-straubing.ics" onclick={trackIcs}>
          Apple/Outlook (.ics)
        </a>
      </div>
    </div>
